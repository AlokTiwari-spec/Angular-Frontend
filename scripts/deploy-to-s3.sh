...
if [ $? -eq 0 ]; then
    ...
    aws s3 cp $BUILD_PATH s3://$AWS_S3_BUCKET/latest --recursive
    echo "Successfully deployed version $TIMESTAMP"
else
    # Get the previous version
    PREVIOUS_VERSION=$(aws s3 ls s3://$AWS_S3_BUCKET/ | grep -v latest | sort | tail -n 1 | awk '{print $2}' | sed 's/\///')
    if [ ! -z "$PREVIOUS_VERSION" ]; then
        # Rollback to the previous version
        aws s3 cp s3://$AWS_S3_BUCKET/$PREVIOUS_VERSION/ s3://$AWS_S3_BUCKET/latest --recursive
